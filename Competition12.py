...
from sklearn.linear_model import Ridge, ElasticNet, Lasso, HuberRegressor, BayesianRidge, PassiveAggressiveRegressor
...
def main():
    ...
    for name, (model, params) in models.items():
        rand_search = RandomizedSearchCV(estimator=model, param_distributions=params, n_iter=10, cv=5, scoring='neg_mean_squared_error', random_state=42)
        rand_search.fit(x_train_split, y_train_split)
    ...
    cv_score = cross_val_score(stacking, x_train, y, cv=cv, scoring='neg_mean_squared_error')
    print(f"Cross-validation RMSE: {(-cv_score.mean()) ** 0.5}")

    # Make predictions on test data
    test_predictions = stacking.predict(x_test)
    out_file(test_predictions, id, 'stacking_predictions7.csv')
